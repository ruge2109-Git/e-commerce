...
export class CategoriaService {
  ...
  async obtenerTodas() {
    ...
    let headers: HttpHeaders = await new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem("token")}`);
    await this.http.get<RtaApi<Categoria>>(`${environment.URL_API}/categorias`, { headers: headers }).toPromise().then(
      (data) => {
        if (data.flag) {
